...
def accuracy(typed, source):
    ...
    numerator = 0
    i = 0
    while i < min(len(source_words), len(typed_words)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    while i > 0:
        i = i - 1
        present_diff = diff_function(typed_word, word_list[i], limit)
        if present_diff <= min:
            min = present_diff
            min_index = i
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1, len(timestamps_per_player[i])):
            temp_time = temp_time + [timestamps_per_player[i][j] - timestamps_per_player[i][j - 1]]
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        fastest_player = 0
        fastest_time = times[0][i]
        for j in player_indices:
            if times[j][i] < fastest_time:
                fastest_time = times[j][i]
                fastest_player = j
        res[fastest_player] = res[fastest_player] + [words[i]]
    return res
